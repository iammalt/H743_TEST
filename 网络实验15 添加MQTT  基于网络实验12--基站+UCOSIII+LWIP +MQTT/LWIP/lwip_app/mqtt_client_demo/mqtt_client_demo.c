...
MQTT_START: 
    ...
    printf("服务器IP地址：%s，端口号：%0d！\n",host_ip,HOST_PORT);
		while(1)
		{
				...
				//等待3秒
				OSTimeDlyHMSM(0,0,0,3000,OS_OPT_TIME_HMSM_STRICT,&err); //延时3000ms
		}
...
MQTT_SEND_START:
    ...
    while(1)
    {//查询消息队列是否有数据需要发送
	  p_errtime=sys_arch_mbox_fetch( &MQTT_Data_Queue,    /* 消息队列的句柄 */
                             (void ** )&send_device_data,      /* 发送的消息内容 */
                             3000); /* 等待时间 3000ms */

      if (p_errtime !=SYS_ARCH_TIMEOUT)
      {
		a=send_device_data->temperature;
		b=send_device_data->humidity;
        printf("a = %f,b = %f\n",a,b);
        //更新数据      
